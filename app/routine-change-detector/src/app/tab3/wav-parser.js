...
function ParsedWave (buffer) {
  ...
  this.fileSize = (new DataView(buffer)).getUint32(FILE_SIZE_OFFSET)
  ...
  this.audioFormat = (new DataView(buffer)).getUint16(AUDIO_FORMAT_OFFSET)
  this.numberOfChannels = (new DataView(buffer)).getUint16(NUMBER_OF_CHANNELS_OFFSET)
  this.sampleRate = (new DataView(buffer)).getUint32(SAMPLE_RATE_OFFSET)
  this.bitsPerSample = (new DataView(buffer)).getUint16(BITS_PER_SAMPLE_OFFSET)
  this.subChunk2Id = buffer.slice(SUBCHUNK2_ID_OFFSET, SUBCHUNK2_ID_OFFSET + 4).toString('utf8')
  this.subChunk2Size = (new DataView(buffer)).getUint32(SUBCHUNK2_SIZE_OFFSET)
  this.data = buffer.slice(DATA_OFFSET, this.subChunk2Size + DATA_OFFSET)
  console.log(this.riff)
  console.log(this.fileSize)
  console.log(this.riffType)
}
