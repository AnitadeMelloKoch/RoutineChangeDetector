...
def main(training_dir, checkpointdir, training_files, train_percent, initial_learning_rate, epochs, batch_size, weighting_action_1, weighting_action_2, weighting_loc, weighting_phone, hidden_1, hidden_2, hidden_3, hidden_4, hidden_5, hidden_6, n_input, n_labels, n_output_action_1, n_output_action_2, n_output_loc, n_output_phone, regulariser_rate):
        ...
        weight_out_action_1 = tf.Variable(tf.random.normal([hidden_6, n_output_action_1]))
        bias_out_action_1 = tf.Variable(tf.random.normal([n_output_action_1]))

        weight_out_action_2 = tf.Variable(tf.random.normal([hidden_6, n_output_action_2]))
        ...
        # output layer
        predicted_y_action_1 = tf.sigmoid(tf.add(tf.matmul(layer_6, weight_out_action_1), bias_out_action_1))
        predicted_y_action_2 = tf.sigmoid(tf.add(tf.matmul(layer_6, weight_out_action_2), bias_out_action_2))
...
if __name__ == '__main__':
        ...
        AP.add_argument("--actions_1",type=int, default=22, help="Number of action labels in output")
        AP.add_argument("--actions_2",type=int, default=12, help="Number of action labels in output")
