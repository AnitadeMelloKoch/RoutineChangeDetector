...
def main(training_dir, checkpointdir, training_files, train_percent, initial_learning_rate, epochs, batch_size, weighting_action_1, weighting_action_2, weighting_loc, weighting_phone, hidden_1, hidden_2, hidden_3, hidden_4, hidden_5, action_1_layer_1, action_2_layer_1, n_input, n_labels, n_output_action_1, n_output_action_2, n_output_loc, n_output_phone, regulariser_rate):
        ...
        weight_action_1_layer_1 = tf.Variable(tf.random.normal([hidden_5, action_1_layer_1]))
        bias_action_1_layer_1 = tf.Variable(tf.random.normal([action_1_layer_1]))

        #initialize weights for action 1 layer 1
        weight_action_2_layer_1 = tf.Variable(tf.random.normal([hidden_5, action_2_layer_1]))
        ...
        # action 1 layer 1
        layer_action_1_layer_1 = tf.nn.sigmoid(tf.add(tf.matmul(layer_5, weight_action_1_layer_1), bias_action_1_layer_1))

        # action 2 layer 1
        layer_action_2_layer_1 = tf.nn.sigmoid(tf.add(tf.matmul(layer_5, weight_action_2_layer_1), bias_action_2_layer_1))
        ...
        print(data_in)
        random_indices = np.arange(data_in.shape[0])
        np.random.shuffle(random_indices)
        data_in = data_in[random_indices]
        action_data_1 = action_data_1[random_indices]
        action_data_2 = action_data_2[random_indices]
        loc_data = loc_data[random_indices]
        phone_data = phone_data[random_indices]
        print(data_in)
